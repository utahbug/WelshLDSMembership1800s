(() => {
  ...
  function organizeResources(name) {
    ...
    info.innerHTML = `<h3>Sources and Evidence</h3>${sourceEvidence}${detailedCardProvenance ? `<ul class="source-evidence-list">${detailedCardProvenance}</ul>` : ""}${nameSources ? `<h3>Sources for branch name</h3><ul class="branch-name-sources">${nameSources}</ul>` : ""}${alternateNames ? `<h3>Alternate names and branch relationships</h3><p>${alternateNames}</p>` : ""}<h3>Registry and technical details</h3><p><a href="branch-registry.html">Consult the branch coverage matrix</a></p><h3>Work remaining</h3><p>${cards.length ? "Review source coverage, transcription status, and discrepancies." : "Locate and connect records for this identified branch."}</p>`;
  ...
  listen(headerSearchButton, "click", openSearch);
  ...
  function routeFromLocation() { const parameters = new URLSearchParams(location.search); const branch = parameters.get("branch"); if (branch && names().includes(branch)) routeBranch(branch, false); else if (parameters.get("view") === "branches") showAllBranches(false); else showHome(false, parameters.has("search")); }
  window.addEventListener("popstate", routeFromLocation);
  function start() { renderDirectory(); const parameters = new URLSearchParams(location.search); const branch = parameters.get("branch"); routeFromLocation(); const imageSequence = Number(parameters.get("image")); const imageFilename = parameters.get("imageFilename"); const collectionId = parameters.get("collection"); if (collectionId && ((Number.isInteger(imageSequence) && imageSequence > 0) || imageFilename)) window.WELSH_OPEN_INDEXED_RECORD?.({ branch, collectionId, imageSequence, imageFilename, view: parameters.get("view") || "single" }); }
