(() => {
  ...
  function branchButton(name, compact = false) { const button = document.createElement("button"); button.type = "button"; button.className = compact ? "picker-branch" : "branch-card"; const collections = related(name); button.innerHTML = `<strong>${name}</strong>${compact ? "" : `<small>${years(detailsFor(name))} · ${collections.length} record collection${collections.length === 1 ? "" : "s"}</small>`}`; button.addEventListener("click", () => routeBranch(name)); return button; }
  ...
  fetch("data/branch-registry.json").then((response) => response.json()).then((data) => { registry = data.registry || []; renderDirectory(); const branch = new URLSearchParams(location.search).get("branch"); if (branch && names().includes(branch)) routeBranch(branch, false); }).catch(() => { count.textContent = "Branch registry could not be loaded."; });
})();
